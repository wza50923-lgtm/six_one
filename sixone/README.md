...
页面顶部「✨ 一键生成（格言5+诗词1+素材5）」：一次生成 **5 条不同类型格言 + 1 首真实诗词（逐字原文+解析） + 5 条一周内素材**，**全部自动勾选**并置顶，完成后**无动画打扰**（不弹层、不变淡），自动滚动到底部**素材本（成稿预览）**展示结果；生成期间界面完全正常可操作（需 API Key，约 30~60 秒）。顶部另有「✕ 取消全部选择」按钮，一键清空所有勾选。
