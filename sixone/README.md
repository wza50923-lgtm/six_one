...
页面顶部「✨ 一键生成（格言5+诗词1+素材5）」：一次生成 **5 条不同类型格言 + 1 首真实诗词（逐字原文+解析） + 5 条一周内素材**，全部自动置顶并勾选，在原有四张卡片里继续编辑/选择即可（需 API Key，约 30~60 秒）。
...
2. 四张卡片对应四类内容：勾选卡片头部复选框 → 展开选择具体条目（也可编辑文本框内容）。**后生成的格言/诗词/素材都会显示在列表最上方**；支持手机/平板浏览（自适应布局）。
